...
FROM (SELECT F.ID                   AS "tisId",
             ...
             TO_NCHAR(F.CONTENT_10) AS "faultDesc",
             TO_NCHAR(F.CONTENT_20) AS "analysisContent",
             TO_NCHAR(F.CONTENT_30) AS "treatmentMeasures",
      ...
      WHERE (F.STATUS = 'FINISHED' OR F.STATUS = 'REPLIED')
       ...
       LEFT JOIN (SELECT FEEDBACK_ID, WMSYS.WM_CONCAT(CASE FROM_SYS WHEN 'TAC' THEN '4S回复人' ELSE '主机厂回复人' END
                                                        || NAME
                  ...
                  FROM (SELECT FR.FEEDBACK_ID, FR.FROM_SYS, FR.CONTENT, U.NAME, U.TELEPHONE, CONTENT_LENGTH
                  ...
                  GROUP BY FEEDBACK_ID HAVING SUM(CONTENT_LENGTH) < 4000) RC ON RC.FEEDBACK_ID = F."tisId"
