SELECT RB.*,
       TO_NCHAR(RE.REPLACED_RB_CODES) AS "replacedRbCodes",
       TO_NCHAR(LNK.LNK_RB_CODES)     AS "lnkRbCodes",
       TO_NCHAR(Q.LNK_QUESTION_IDS)   AS "lnkQuestionIds",
       TO_NCHAR(P.LNK_PART_NOS)       AS "lnkPartNos"
FROM (SELECT R.*
      FROM (SELECT RB.ID                                 AS "tisMid",
       ...
       LEFT JOIN (SELECT T_RB_REPLACED.RB_ID, SUBSTR(WMSYS.WM_CONCAT(T_RB.CODE), 1, 2000) REPLACED_RB_CODES
                  FROM T_RB_REPLACED
                         LEFT JOIN T_RB ON T_RB_REPLACED.REPLACED_RB_ID = T_RB.ID
                  GROUP BY T_RB_REPLACED.RB_ID)RE ON RB."tisMid" = RE.RB_ID
       LEFT JOIN (SELECT T_RB_LNK_BULLETIN.RB_ID, SUBSTR(WMSYS.WM_CONCAT(T_RB.CODE), 1, 2000) LNK_RB_CODES
       ...
       LEFT JOIN (SELECT RB_ID, SUBSTR(WMSYS.WM_CONCAT(LNK_QUESTION_ID), 1, 2000) LNK_QUESTION_IDS
                  FROM T_RB_LNK_QUESTION
                  GROUP BY RB_ID)Q ON RB."tisMid" = Q.RB_ID
       LEFT JOIN (SELECT RB_ID, SUBSTR(WMSYS.WM_CONCAT(LNK_PART_NO), 1, 2000) LNK_PART_NOS
                  FROM T_RB_LNK_PART
